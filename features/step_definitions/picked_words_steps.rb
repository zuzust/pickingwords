...
When /^I look for nonexistent pick '(.*?)' from '(.*?)' to '(.*?)'$/ do |pick,from,to|
  step "I look for pick '#{pick}' from '#{from}' to '#{to}'"
end

When /^I look for existing pick '(.*?)' from '(.*?)' to '(.*?)'$/ do |pick,from,to|
  step "I look for pick '#{pick}' from '#{from}' to '#{to}'"
end

When /^I look for pick '(.*?)' from '(.*?)' to '(.*?)'$/ do |pick,from,to|
  fill_in "name", with: pick
...
Then /^I should see a missing name error message$/ do
  page.body.should =~ /Name can't be blank/i
end

Then /^I should see a invalid name error message$/ do
  page.body.should =~ /Name is not a dictionary word/i
end

Then /^I should see a same source and target locales error message$/ do
  page.body.should =~ /Make sure that source and target languages are different/i
end

Then /^I should see a nothing to show message$/ do
  page.should have_content("No picks to show")
end

Then /^I should only see the pick '(.*?)' listed$/ do |pick|
  step "I should see the pick '#{pick}'"
  page.has_css? 'div.pw-word', count: 1
end

Then /^I should see the pick '(.*?)'$/ do |pick|
  page.should have_content(pick) 
end
